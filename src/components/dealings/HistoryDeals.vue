<template>
    <div>
                ...
                <tr v-for="(item, index) in deals.slice(getFirstItemPage , (10*activePage))" :key="index">
                    ...
                    <td>
                        <v-chip color="green" label>
                            <v-icon size="x-small" start icon="mdi-arrow-up"></v-icon>{{ item.kurs }}
                        </v-chip>
                    </td>
                    <td>
                    ...
                    <td>
                        <v-icon size="x-small" color="blue" class="mr-1">mdi-check-decagram</v-icon>
                        {{ item.merchant.name }}
                    </td>
                    <td>
                        <v-chip color="green" label>
        ...
        <div class="d-flex justify-end align-center" v-if="deals.length">
            <v-btn variant="text" size="x-small" class="elevation-0" icon="mdi-arrow-left" color="white"
                @click="changePage(-1)" :disabled="activePage === 1"></v-btn>
            <p class="mx-4 text-white">{{activePage}} <span class="mx-2">из</span> {{Math.ceil(deals.length/10)}}</p>
            <v-btn variant="text" size="x-small" class="elevation-0" icon="mdi-arrow-right" color="white"
                @click="changePage(1)" :disabled="activePage === Math.ceil(deals.length/10)"></v-btn>
        </div>
    </div>
</template>
